...
public class PermissionsCommand extends Command implements IUtilCommand {
    ...
    public void onInvoke(Guild guild, TextChannel channel, Member invoker, Message message, String[] args) {
        ...
        List<Permission> sortperms = permurl;
        try {
             hasteurl = TextUtils.postToHastebin(sortperms.toString().replace("_", " ").replace("["," ").replace("]", " ").toLowerCase(), true) + ".perms";
        }
        ...
        eb.addField("Permissions for" + target.getEffectiveName(), sortperms.toString().replace("_", " ").replace("["," ").replace("]", " ").toLowerCase(), true);
